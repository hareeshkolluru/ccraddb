...
{ $i18=0;
while($_POST["txtSiteName_temp"][18][$i18])
{

$i19=0;
	   $sqlquerygetchannellist="select ".$_POST["txtSiteName_temp"][18][$i18]." from channelsTable";/*".$table3;*/
	   $getchannellist = mysql_query($sqlquerygetchannellist);
       $m=0;
	   while ($channelnumber=mysql_fetch_array($getchannellist)){
	   if($channelnumber[0]){
	   $_POST["txtSiteName_temp"][19][$m] = $channelnumber[0];
	   $m++;}
       }
	   /*echo "$m";*/
...
while($m!=0)
		{
				...
				case 18: $_POST["txtSiteName"][$column_no]=$_POST["txtSiteName_temp"][$column_no][$i18];
						break;
				case 19: $_POST["txtSiteName"][$column_no]=$_POST["txtSiteName_temp"][$column_no][$i19];
